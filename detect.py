...
from calculate_reward import calculate_reward_new, calculate_score, calculate_average_score
from init_anchor import generate_initial_anchors, merge_anchor, generate_more_initial_anchors
...
class PPOActor(nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.isnan(features).any() or torch.isinf(features).any():
            print("⚠️ Feature extractor输出非法值（NaN/Inf）")
...
def train_agent(df_origin_list, anchor_lists, agent, num_epochs=20, stop_thresh=0.002, max_no_change_steps=6):
...
def process_csv_for_init(file_path):
    ...
    filename = os.path.basename(file_path).split('.csv')[0]  # 返回文件名，主要是保存结果时用的
    print(f"加载{filename}")
...
if __name__ == "__main__":
    ...
    print("---------------------加载测试集--------------------------")
    dataset_path = "dataset/lowSNR"

    csv_files=[]
    for f in os.listdir(dataset_path):
        if f.endswith(".csv"):
            csv_files.append(os.path.join(dataset_path, f))
    ...
    # 拆分结果
    df_origin_list, anchor_lists,filename_list = zip(*results)
    ...
    for index,(df,anchor_list,filename) in enumerate(zip(df_origin_list,anchor_lists,filename_list)):

        img_list=[]
        img=plot_greyscale_for_singledf_with_anthor_and_score(df, [], saveimg=False)
        img_list.append(img)

        optimized_anchors = test_agent(df, anchor_list, agent)
        merged_anchors=merge_anchor(optimized_anchors, iou_threshold=0.01)
        img=plot_greyscale_for_singledf_with_anthor_and_score(df, merged_anchors,saveimg=False)
        img_list.append(img)
        average_score=calculate_average_score(merged_anchors,df)
        print(f"average_score:{average_score}")
        average_score_threhold=0.5
        while average_score>average_score_threhold:
            anchor_list1=generate_more_initial_anchors(df, exist_anchors=merged_anchors, num_anchors=30)#在原来锚框的基础上生成新的锚框
            optimized_anchors = test_agent(df, anchor_list1, agent)
            merged_anchors = merge_anchor(merged_anchors+optimized_anchors, iou_threshold=0.01)
            average_score = calculate_average_score(merged_anchors, df)
            print(f"average_score:{average_score}")
            if average_score>average_score_threhold:#防止把太抽象的框加进去
                img=plot_greyscale_for_singledf_with_anthor_and_score(df, merged_anchors,saveimg=False)
                img_list.append(img)
            if len(img_list) > 20:
                break  # 也不能一直没完没了地调整下去
        save_as_gif(img_list, duration=1000, gif_name=f"gifs/all/{filename}.gif")